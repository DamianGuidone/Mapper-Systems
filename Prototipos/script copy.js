//#region Diálogo para explorar archivos
  ...
  function loadFolderTree(path = "") {
    ...
    fetch(`/api/archive/${encodeURIComponent(path)}`)
      .then(res => res.text())
      .then(html => {
...
//#region Gestión de formulario de carga de funciones
  ...
  function addNewRegistro() {
    ...
    fetch(`/api/registros?nodo_id=${selected.id}`)
        .then(res => res.json())
        .then(entries => {
  ...
  function deleteFunction(id) {
      ...
      fetch(`/api/delete-funcion?id=${id}`, { method: 'DELETE' })
  ...
  function loadFunctionsTable(nodeId) {
    fetch(`/api/registros?nodo_id=${nodeId}`)
      .then(res => res.json())
      .then(entries => {
  ...
  function filterRegistros(value) {
    ...
    let url = `/registros?nodo_id=${selected.id}`;
  ...
  function sortRegistros(order = "asc") {
    ...
    const url = `/api/registros?nodo_id=${selected.id}&order=${order}`;
    loadFunctionsTableFromUrl(url);
  }
  ...
  function saveEditedFunction() {
    ...
    fetch('/api/registro', {
      ...
      reindexOrder(selected.id); // Llama a reindexar órdenes
      disableRegistroForm();
  ...
  function deleteFunction(id) {
    ...
    fetch(`/api/delete-registro?id=${id}`, { method: 'DELETE' })
      .then(() => {
  ...
  function deleteSelectedRegistro() {
    ...
    fetch(`/api/delete-registro?id=${currentRegistro.id}`, { method: 'DELETE' })
      .then(() => {
          showToast("Registro eliminado");
          reindexOrder($('#jstree').jstree().get_selected(true)[0].id);
      });
  }
  ...
  function reindexOrder(nodo_id) {
    fetch("/api/reindex-orden", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nodo_id })
    })
    .then(res => res.json())
    .then(data => {
      if (data.success) {
        console.log(`✅ Órdenes reindexados (${data.total_registros} registros)`);
      } else {
        console.error("[ERROR] No se pudieron reindexar órdenes:", data.details);
      }
    })
    .catch(err => {
      console.error("[ERROR] Al llamar a /api/reindex-orden:", err);
    });
  }
...
//#region Gestión de Árbol de nodos
  ...
  function saveTree() {
    ...
    .then(data => {
      if (data.success) {
        showToast("Árbol guardado correctamente");
      } else {
        showToast("Error al guardar el árbol", "danger");
      }
    })
    .catch(error => {
      console.error("[ERROR] Al guardar árbol:", error);
      showToast("No se pudo guardar el árbol", "danger");
    });
  }
  ...
  function loadTreeFromServer() {
    fetch("/api/nodos")
        ...
        .then(nodes => {
            ...
            // Aplicar filtros, historial, etc.
            applyNodeFilters($('#jstree').jstree(true));
        });
}
...
//#region Gestión de Tarjeta de Detalles de Nodo
  ...
  //#region Preview
    ...
    function openSqlFile() {
      ...
      fetch(`/api/open-sql?path=${encodeURIComponent(path)}`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
        ...
        .catch(err => {
          ...
          showToast("No se pudo abrir el archivo", "danger");
        });
    }
  ...
  //#region Historial de nodos
    ...
    function addHistoryEntry(nodeId, campo, valorAnterior, valorNuevo) {
      ...
      fetch("/api/historial_registro", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(entry)
      }).then(res => res.json())
        .then(() => showToast(`Historial actualizado`))
     ...
     * 
     */
    function undoChange(fecha, campo, anterior, nuevo) {
      const selected = $('#jstree').jstree().get_selected(true)[0];
      if (!selected || !currentRegistro) return;

      const registro_id = currentRegistro.id;
      ...
      fetch("/api/historial_registro", {
        ...
        showToast(`"${campo}" revertido a: "${anterior}"`);
        reloadDetailsCard();
      });
    }

    function toggleRegistroHistory() {
      const visible = $('#registro-history-container').is(":visible");
      $('#registro-history-container').toggle(!visible);
      $('#registro-history-toggle-icon')
        .toggleClass("fa-eye", !visible)
        .toggleClass("fa-eye-slash", visible);
    }

    function loadRegistroHistory(registroId) {
      fetch(`/api/historial_registro/${registroId}`)
        .then(res => res.json())
        .then(entries => {
          const container = $('#registro-history');
          container.empty();

          if (entries.length === 0) {
            container.append('<li class="list-group-item text-muted">Sin historial</li>');
            return;
          }
          ...
          entries.forEach(entry => {
            ...
            container.append(`
              <li class="list-group-item d-flex align-items-center">
                ...
                <button class="btn btn-sm btn-outline-danger ms-2" onclick='undoChange("${entry.fecha}", "${entry.campo}", "${entry.anterior}", "${entry.nuevo}")'>
        ...
        .catch(() => {
          $('#registro-history').empty().append('<li class="list-group-item text-muted">No se pudo cargar el historial</li>');
        });
    }
...
//#region Funciones de soporte
  ...
  /** Carga un archivo desde /api/read-sql?path=...
   * 
  ...
  */
  function loadFileFromArchive(targetElementId) {
    ...
    fetch(`/api/read-sql?path=${encodeURIComponent(selectedFilePath)}`)
      .then(res => {
        if (!res.ok) throw new Error("Archivo no encontrado");
  ...
  //#region Eventos
    ...
    $(document).on("click", ".fas.fa-folder", function () {
      ...
      if (!isExpanded) {
        ...
        fetch(`/api/archive/${encodeURIComponent(newPath)}`)
          .then(res => res.text())
          .then(html => {
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, "text/html");
    ...
    $(document).on("click", "#registros-table tr", function () {
      ...
      // Cargar historial del registro seleccionado
      fetch(`/api/historial_registro/${registro.id}`)
        .then(res => res.json())
        .then(entries => {
