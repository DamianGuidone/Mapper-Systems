...
import { registrosApi, filesApi } from "../services/api" // Actualizado
...
export const TablaRegistros: React.FC<TablaRegistrosProps> = ({ expanded = false }) => {
  ...
  const cargarRegistros = async () => {
    ...
    setLoading(true)
    try {
      const response = await registrosApi.getRegistros(selectedNode.id)
  ...
  const guardarRegistro = async () => {
    ...
    try {
      const registroToSave: Registro = {
        ...formData,
        nodo_id: selectedNode.id,
        proyecto_id: currentProject.id,
        archivo: formData.archivo || "",
        orden: formData.orden || registros.length + 1,
      } as Registro

      await registrosApi.saveRegistro(registroToSave)
  ...
  const eliminarRegistro = async (registroId: number) => {
    ...
    try {
      await registrosApi.deleteRegistro(registroId)
      await cargarRegistros()
  ...
  const duplicarRegistro = (registro: Registro) => {
    setSelectedRegistro(null)
    setFormData({
      ...registro,
      id: undefined,
      orden: (registro.orden || 0) + 1,
    })
    setFormOpen(true)
  }

  const previsualizarArchivo = async (filePath: string) => {
    if (!filePath) return
  ...
  const abrirArchivo = async (filePath: string) => {
    if (!filePath) return
  ...
  const resetForm = () => {
    setFormData({
      ...
      funcion: "",
      descripcion: "",
      orden: registros.length + 1,
    })
    setSelectedRegistro(null)
  }

  const registrosFiltrados = registros.filter((registro) =>
    Object.values(registro).some((value) => value?.toString().toLowerCase().includes(searchTerm.toLowerCase())),
  )
  ...
  return (
    <>
      <Card>
        <CardContent>
              ...
              <TableBody>
                {registrosFiltrados.map((registro, index) => (
                  <TableRow key={registro.id} hover>
                    <TableCell>
                      <Chip label={registro.orden || index + 1} size="small" color={nodeInfo.color as any} />
                    </TableCell>
                    <TableCell>
                        ...
                        {registro.archivo && (
                          <Box>
                            <Tooltip title="Ver código">
                              <IconButton size="small" onClick={() => previsualizarArchivo(registro.archivo)}>
                                <Code fontSize="small" />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="Abrir archivo">
                              <IconButton size="small" onClick={() => abrirArchivo(registro.archivo)}>
                    ...
                    <TableCell>
                      <Typography variant="body2">{registro.ubicacion || "-"}</Typography>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell align="center">
                      <Box sx={{ display: "flex", gap: 0.5 }}>
                        <Tooltip title="Editar">
                          <IconButton size="small" onClick={() => editarRegistro(registro)}>
                            <Edit fontSize="small" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="Duplicar">
                          <IconButton size="small" onClick={() => duplicarRegistro(registro)}>
                        ...
                        <Tooltip title="Eliminar">
                          <IconButton
                            size="small"
                            onClick={() => registro.id && eliminarRegistro(registro.id)}
                            color="error"
                          >
          ...
          <Box sx={{ mt: 2, display: "flex", justifyContent: "between", alignItems: "center" }}>
            ...
            {registros.length > 0 && (
              <Button size="small" onClick={() => registrosApi.reordenarRegistros(selectedNode.id)}>
                Reordenar
              </Button>
            )}
          </Box>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>
          {selectedRegistro ? "Editar Registro" : "Nuevo Registro"} - {nodeInfo.name}
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button onClick={guardarRegistro} variant="contained">
